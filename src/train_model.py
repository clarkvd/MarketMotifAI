...
def train_optuna_xgboost_model(
    ...
    """
    Trains an XGBoost binary classification model using Optuna for hyperparameter tuning,
    to predict whether a fragility event (volatility spike) will occur in the next 5 days.

    The target label is assigned based on whether a peak event occurs within 5 calendar days
    of a given row's date. Both core and motif similarity features are used for prediction.

    Parameters:
        df_model (pd.DataFrame): Feature matrix including motif similarity columns and dates.
        df_summits (pd.DataFrame): Table of peak (fragile) summit dates for labeling.
        significant_clusters (list): List of cluster IDs to use for motif similarity features.
        n_trials (int): Number of Optuna trials to run (default = 150).
        verbose (bool): Whether to display progress, metrics, and plots (default = True).

    Returns:
        Tuple:
            - results (dict): Dictionary with trained model, predictions, metrics, and thresholds.
            - df_test (pd.DataFrame): Test set used for evaluation, aligned with predictions.
    """

    # --- Normalize date columns and label target (fragility within 5 days) ---
    ...
    # --- Define Optuna objective function for AUC maximization ---
    def objective(trial):
        params = {
